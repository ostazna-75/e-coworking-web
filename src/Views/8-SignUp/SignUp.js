...
const SignUp = ({ hide, setUser }) => {
  const {
    ...
    errorMessage,
    setErrorMessage
  } = useData({ setUser, hide });

  return (
    <TemplateSign>
      <div style={{ position: "absolute", top: 30, right: 30 }} onClick={hide}>
        X
      </div>
      {/* goupe Sign Up  */}
      <WordingSignGroup>
...
const useData = ({ setUser, hide }) => {
  ...
  const submit = async e => {
    ...
    try {
      response = await axios.post(`user/${newinfo ? "signup" : "signin"}`, {
        password,
        email,
        username: newinfo ? username : undefined
      });
...
/* -------- FIN nouvelle fonction pour custom Hooks ----------*/
